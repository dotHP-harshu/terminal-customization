...
export async function POST(request: Request) {
    ...
    if(!messages || !Array.isArray(messages)) return Response.json({ message: "Messages not found" });
  ...
  try {
    ...
    if(!res.choices){return Response.json({message:"Error on generating res from ai."})}
